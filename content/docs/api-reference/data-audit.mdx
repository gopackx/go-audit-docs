...
- For DELETEs, persists `OldValues` and stores `null` for
  `new_values`. (The GORM adapter populates `OldValues` from the row
  being deleted; raw callers should supply it.)
